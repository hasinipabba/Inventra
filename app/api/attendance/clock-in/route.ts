...
import { clockIn } from "@/lib/attendance-db";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const record = await clockIn({ id: staff.id, name: staff.name, email: staff.email }, date, new Date().toISOString());
    return NextResponse.json(record, { status: 201 });
  } catch (err) {
    if (err instanceof AttendanceError) {
      return NextResponse.json({ error: err.message, code: err.code }, { status: err.status });
    }
    console.error("POST /api/attendance/clock-in failed:", err);
    return NextResponse.json({ error: "Couldn't clock you in right now. Please try again." }, { status: 500 });
  }
}
